<?php
    ...
    class InstallHelper
    {
        ...
        public static function doInstall(&$oContainer, &$aData)
        {
            ...
            $checkResult = self::_checkDatabaseCredentials();
            if ($checkResult === true) {
                ...
                throw new \Exception(self::$_container->get("translator")->trans("install.bad_database_credentials") . "<br>Error: " . $checkResult);
            }
        }
        ...
        private static function _checkDatabaseCredentials()
        {
            ...
            try {
                ...
                if (substr(self::$_data['dbHost'], 0, 1) == "/") {
                    $sDsn = 'mysql:unix_socket=' . self::$_data['dbHost'] . ';dbname=' . self::$_data['dbName'] . ';charset=UTF8';
                } else {
                    $sDsn = 'mysql:host=' . self::$_data['dbHost'] . ';port=' . self::$_data['dbPort'] . ';dbname=' . self::$_data['dbName'] . ';charset=UTF8';
                }

                $bResult           = true;
                self::$_connection = new \PDO($sDsn, self::$_data['dbUser'], self::$_data['dbPassword']);

                self::$_data['dbDsn'] = $sDsn;
            ...
            if (substr(self::$_data['dbHost'], 0, 1) == "/") {
                $sYmlDump = array(
                    'parameters' => array(
                        ...
                        'database_dsn'      => self::checkEmpty(self::$_data['dbDsn']),
        ...
        private static function clearCache()
        {
            ...
            $input  = new ArgvInput(array(
                                        'assets:install',
                                        self::$_container->get('kernel')->getRootDir() . "/../web"
                                    ));
